defmodule CodeMySpec.ContextCodingSessions.Steps.Finalize do
  @moduledoc """
  Completes the context coding session by committing all implementation code from child
  ComponentCodingSession workflows, pushing the implementation branch to remote, and marking
  ...
  def get_command(%Scope{} = _scope, session, _opts) do
    with {:ok, context_component} <- get_context_component(session),
         ...
         {:ok, files} <- collect_implementation_files(child_sessions) do
      branch_name = Utils.branch_name(session)
      command_string = build_git_command(context_component, child_sessions, files, branch_name)

      metadata = %{
        branch_name: branch_name,
        committed_files: length(files)
      }
  ...
  defp collect_implementation_files(child_sessions) do
    files =
      child_sessions
  ...
  defp build_git_command(context_component, child_sessions, files, branch_name) do
  ...
  defp build_commit_message(context_component, child_sessions) do
    title = "Implement #{context_component.name} context"
