defmodule CodeMySpec.ContextCodingSessions.Steps.SpawnComponentCodingSessions do
  @moduledoc """
  Creates child ComponentCodingSession records for each component within the target context.

  Returns a spawn_sessions command containing child_session_ids in metadata, enabling the
  client to orchestrate parallel autonomous implementation generation across all context components.
  """
  ...
  defp validate_child_sessions_type(child_sessions) do
    invalid_sessions =
      Enum.reject(child_sessions, fn session ->
        session.type == CodeMySpec.ComponentCodingSessions
      end)
    ...
    case invalid_sessions do
      ...
      [session | _] ->
        {:error,
         "Invalid child session type: expected ComponentCodingSessions, got #{inspect(session.type)}"}
    end
  end
  ...
  defp create_new_child_sessions(
    ...
    with {:ok, child_components} <- get_child_components(scope, context_component) do
      branch_name = Utils.branch_name(parent_session)
      create_child_sessions(scope, parent_session, child_components, branch_name)
    end
  end
  ...
  defp create_child_sessions(
    ...
    results =
      Enum.map(child_components, fn component ->
        create_child_session(scope, parent_session, component, branch_name)
      end)
  ...
  defp validate_child_sessions_status(child_sessions) do
    ...
    case {active, failed, cancelled} do
      ...
      {_, [_ | _] = failed, _} ->
        names = Enum.map(failed, & &1.component.name) |> Enum.join(", ")
        {:error, "Child sessions failed: #{names}"}
