defmodule CodeMySpec.ContextTestingSessions.Steps.SpawnComponentTestingSessions do
  @moduledoc """
  Creates child ComponentTestingSession records for each component within the target context.

  Returns a spawn_sessions command containing child_session_ids in metadata, enabling the
  client to orchestrate parallel autonomous test generation across all context components.
  """
  ...
  defp validate_child_sessions_type(child_sessions) do
    invalid_sessions =
      Enum.reject(child_sessions, fn session ->
        session.type == CodeMySpec.ComponentTestSessions
      end)
    ...
    case invalid_sessions do
      ...
      [session | _] ->
        {:error,
         "Invalid child session type: expected ComponentTestSessions, got #{inspect(session.type)}"}
    end
  end
  ...
  defp create_new_child_sessions(
    ...
    with {:ok, child_components} <- get_child_components(scope, context_component) do
      create_child_sessions(scope, parent_session, child_components)
    end
  end
  ...
  defp create_child_sessions(
    ...
    results =
      Enum.map(child_components, fn component ->
        create_child_session(scope, parent_session, component)
      end)
