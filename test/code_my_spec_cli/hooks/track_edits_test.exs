defmodule CodeMySpecCli.Hooks.TrackEditsTest do
  use CodeMySpec.DataCase, async: true

  import ExUnit.CaptureIO

  alias CodeMySpecCli.Hooks.TrackEdits
  ...
  # ============================================================================
  # run_and_output/1 - Outputs JSON to stdout
  # ============================================================================

  describe "run_and_output/1" do
    test "outputs {} for all cases (never blocks tool use)" do
      session_id = Ecto.UUID.generate()

      # Test with Write tool
      write_output =
        capture_io(fn ->
          TrackEdits.run_and_output(write_tool_input(session_id, "/path/to/file.ex"))
        end)

      assert String.trim(write_output) == "{}"

      # Test with Edit tool
      edit_output =
        capture_io(fn ->
          TrackEdits.run_and_output(edit_tool_input(session_id, "/path/to/file.ex"))
        end)

      assert String.trim(edit_output) == "{}"

      # Test with Read tool (non-edit)
      read_output =
        capture_io(fn ->
          TrackEdits.run_and_output(read_tool_input(session_id, "/path/to/file.ex"))
        end)

      assert String.trim(read_output) == "{}"

      # Test with no session
      no_session_output =
        capture_io(fn ->
          TrackEdits.run_and_output(tool_input_no_session("Write", "/path/to/file.ex"))
        end)

      assert String.trim(no_session_output) == "{}"

      # Test with missing file_path
      missing_path_output =
        capture_io(fn ->
          TrackEdits.run_and_output(tool_input_missing_file_path(session_id, "Write"))
        end)

      assert String.trim(missing_path_output) == "{}"
    end
  end
end
