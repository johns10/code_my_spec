defmodule CodeMySpec.ContextCodingSessions.Steps.SpawnComponentCodingSessionsTest do
  ...
  defp create_child_session(scope, parent_session, component, opts) do
    session_fixture(scope, %{
      ...
      state: Keyword.get(opts, :state, %{parent_branch_name: "test-branch"})
    })
  end
  ...
  describe "get_command/3" do
    test "returns spawn_sessions command with child_session_ids in metadata" do
      ...
      assert {:ok, %Command{} = command} =
               SpawnComponentCodingSessions.get_command(scope, parent_session, [])

      assert command.module == SpawnComponentCodingSessions
    ...
    test "creates child sessions for each component with correct attributes" do
      ...
      Enum.each(child_session_ids, fn session_id ->
        ...
        assert session != nil
        assert session.type == CodeMySpec.ComponentCodingSessions
    ...
    test "returns existing child_session_ids when child sessions already exist (idempotent)" do
      ...
      assert {:ok, %Command{} = second_command} =
               SpawnComponentCodingSessions.get_command(scope, parent_session_reloaded, [])
    ...
    test "validates existing child session types match ComponentCodingSessions" do
      scope = full_scope_fixture()
      {context_component, children} = setup_context_with_children(scope)
      parent_session = create_parent_session(scope, context_component)
    ...
    test "returns error when existing child sessions have invalid type" do
      ...
      Enum.each(children, fn child ->
        create_child_session(scope, parent_session, child, type: CodeMySpec.ComponentTestSessions)
      end)
      ...
      assert error_message =~
               "Invalid child session type: expected ComponentCodingSessions, got"

      assert error_message =~ "ComponentTestSessions"
    end

    test "returns error when parent session not found during idempotency check" do
      scope = full_scope_fixture()
      ...
      assert {:error, "Context component not found"} =
               SpawnComponentCodingSessions.get_command(scope, fake_session, [])
    end
  end
  ...
  describe "handle_result/4" do
    ...
    test "returns error when parent session not found" do
      ...
      assert {:error, "Session not found"} =
               SpawnComponentCodingSessions.handle_result(scope, fake_session, result, [])
    end
