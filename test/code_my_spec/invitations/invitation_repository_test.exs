defmodule CodeMySpec.Invitations.InvitationRepositoryTest do
  ...
  describe "create_invitation/2" do
    ...
    test "creates invitation with valid attributes", %{scope: scope, user: user, account: account} do
      ...
      assert {:ok, %Invitation{} = invitation} = InvitationRepository.create_invitation(scope, attrs)
  ...
  describe "get_invitation/2" do
    ...
    test "returns nil when invitation belongs to different account", %{user: user, invitation: invitation} do
      other_account = account_with_owner_fixture(user)
      scope = %Scope{user: user, active_account_id: other_account.id}
  ...
  describe "update_invitation/3" do
    ...
    test "updates invitation with valid attributes", %{scope: scope, invitation: invitation} do
      ...
      assert {:ok, %Invitation{} = updated} = InvitationRepository.update_invitation(scope, invitation, attrs)
    ...
    test "returns error with invalid attributes", %{scope: scope, invitation: invitation} do
      ...
      assert {:error, changeset} = InvitationRepository.update_invitation(scope, invitation, attrs)
      assert %{email: ["has invalid format"]} = errors_on(changeset)
    end
  end
  ...
  describe "by_account/2" do
    setup do
      ...
      %{account1: account1, account2: account2, invitation1: invitation1, invitation2: invitation2}
    end
  ...
  describe "cleanup_expired_invitations/1" do
    setup do
      ...
      Repo.update!(Invitation.changeset(old_expired, %{
  ...
  describe "count_pending_invitations/1" do
    ...
    test "counts pending invitations", %{scope: scope} do
      count = InvitationRepository.count_pending_invitations(scope)
      assert count == 2
    end

    test "returns 0 when scope has no active account" do
      scope = %Scope{user: user_fixture(), active_account_id: nil}
      assert InvitationRepository.count_pending_invitations(scope) == 0
    end
  end
end
