defmodule CodeMySpec.ContextCodingSessionsTest do
  ...
  describe "context coding session workflow" do
    setup do
      user = user_fixture()
    ...
    @tag timeout: 300_000
    test "executes complete context coding workflow", %{
      scope: scope,
      accounts_context: accounts_context
    } do
      Sessions.subscribe_sessions(scope)

      # Create context coding session
      {:ok, session} =
      ...
      # Step 2: SpawnComponentCodingSessions
      {:ok, session} = Sessions.next_command(scope, session.id)
      [interaction | _] = session.interactions
      assert interaction.command.module == SpawnComponentCodingSessions
      ...
      # Mark all child sessions as complete and create implementation files
      ...
      File.mkdir_p!("lib/my_app/accounts")
      File.write!("lib/my_app/accounts/user.ex", "defmodule MyApp.Accounts.User do\nend")

      File.write!(
        "lib/my_app/accounts/user_repository.ex",
        "defmodule MyApp.Accounts.UserRepository do\nend"
      )
    ...
    test "handles validation failure and retry for incomplete child sessions", %{
      ...
      # Step 2: SpawnComponentCodingSessions - first attempt
      {:ok, session} = Sessions.next_command(scope, session.id)
      [interaction | _] = session.interactions
      child_session_ids = interaction.command.metadata["child_session_ids"]
      ...
      # Create only one implementation file
      File.mkdir_p!("lib/my_app/accounts")
      File.write!("lib/my_app/accounts/user.ex", "defmodule MyApp.Accounts.User do\nend")
      ...
      # Step 3: Should retry SpawnComponentCodingSessions
      {:ok, session} = Sessions.next_command(scope, session.id)
      [interaction | _] = session.interactions
      assert interaction.command.module == SpawnComponentCodingSessions
    ...
    @tag timeout: 300_000
    test "handles validation failure when unit tests fail", %{
      scope: scope,
      accounts_context: accounts_context
    } do
      Sessions.subscribe_sessions(scope)
      ...
      # SpawnComponentCodingSessions - first attempt
      {:ok, session} = Sessions.next_command(scope, session.id)
      [interaction | _] = session.interactions
      child_session_ids = interaction.command.metadata["child_session_ids"]

      # Mark all child sessions complete and create all implementation files
      for child_id <- child_session_ids do
        child_session = Sessions.get_session!(scope, child_id)
        {:ok, _} = Sessions.update_session(scope, child_session, %{status: :complete})
      end
    ...
    test "validates all components have corresponding child sessions", %{
      ...
      # SpawnComponentCodingSessions should create child sessions for all 3 components
      {:ok, session} = Sessions.next_command(scope, session.id)
      [interaction | _] = session.interactions
      child_session_ids = interaction.command.metadata["child_session_ids"]
      ...
      for child_id <- child_session_ids do
        child_session = Sessions.get_session!(scope, child_id)
        assert child_session.type == CodeMySpec.ComponentCodingSessions
    ...
    test "stores branch name in session state", %{
      ...
      # Initialize step should create branch and store name
      {:ok, session} = Sessions.next_command(scope, session.id)
      [interaction | _] = session.interactions
      assert interaction.command.module == Initialize
    ...
    test "finalizes by committing and marking session complete", %{
      ...
      # SpawnComponentCodingSessions - complete successfully
      {:ok, session} = Sessions.next_command(scope, session.id)
      [interaction | _] = session.interactions
      child_session_ids = interaction.command.metadata["child_session_ids"]
      ...
      # Create all implementation and test files
      File.mkdir_p!("lib/my_app/accounts")
      File.write!("lib/my_app/accounts/user.ex", "defmodule MyApp.Accounts.User do\nend")
