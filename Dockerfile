...
RUN apt-get update \
  && apt-get install -y --no-install-recommends libstdc++6 openssl libncurses5 locales ca-certificates git \
  && rm -rf /var/lib/apt/lists/*
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/code_my_spec ./
